...
**Dependencies:** `spu_quartic_vacuum_Z4.md`, `Dynamic_Selection_n3_in_Z4_Medium.md`, `IR_Dominance_Calculator.py`
...
| **0.65** (natural) | $9.55 \times 10^{16}$ | 5.31 | 16.98 | ✅ Accessible |
...
2. **Zero-Mode Solver**: Verify $\text{Ind}(\not{D}) = 3$ in the $Z_3$-locked background.
3. **Energy Minimization**: Confirm $E_3 < E_1, E_2, E_4$ under the running $\kappa_3(\mu), \kappa_4(\mu)$.

### 6.3 Script Validation

The companion script `IR_Dominance_Calculator.py` implements the corrected formula and produces:
...
**Usage example:**
```python
from IR_Dominance_Calculator import running_ratio, crossover_scale

# Evaluate ratio at μ = 10^15 GeV with R₀ = 0.65
R_val = running_ratio(mu=1e15, R0=0.65)
print(f"R(10¹⁵ GeV) = {R_val:.4f}")

# Compute crossover scale
mu_star = crossover_scale(R0=0.65)
