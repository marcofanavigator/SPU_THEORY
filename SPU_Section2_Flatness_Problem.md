...
$$|\Omega - 1|_{t_{\text{Pl}}} \lesssim 10^{-60}$$
...
$$H^2 = \frac{\rho}{3M_{\text{Pl}}^2} - \frac{k}{a^2}$$
...
$$\rho_{\text{vac}} \leq \rho_{\text{SP}} \equiv \Lambda_{\text{SP}}^4$$

This is a hard bound imposed by the finite fermionic capacity $N_f^{\text{eff}} \approx 127.37$. No physical configuration can exceed this energy density within the SPU framework, because there are no additional fermionic degrees of freedom available to support it.

The physical consequence for the Friedmann equation is that the effective Hubble parameter at the GUT scale is not $H_{\text{GUT}} \sim M_{\text{GUT}}^2/M_{\text{Pl}}$ (the standard value) but is regulated by $\Lambda_{\text{SP}}$:

$$H_{\text{SP}} \sim \frac{\Lambda_{\text{SP}}^2}{M_{\text{Pl}}} = \frac{N_f^{\text{eff}} \cdot M_{\text{GUT}}^2}{M_{\text{Pl}}}$$

Numerical evaluation:

$$H_{\text{SP}} = \frac{127.37 \times (10^{16})^2}{2.4 \times 10^{18}} \text{ GeV} \approx \frac{1.274 \times 10^{34}}{2.4 \times 10^{18}} \text{ GeV} \approx 5.3 \times 10^{15} \text{ GeV}$$


### 2.3 Suppression of $|\Omega - 1|$ by the SPU Vacuum Structure
...
In SPU, the scale factor at the GUT transition is set by the condition that the physical energy density equals $\Lambda_{\text{SP}}^4$. The ratio of the curvature term to the energy density term is:

$$\frac{|k|/a^2}{H^2} = \frac{|k| M_{\text{Pl}}^2}{a^2 \Lambda_{\text{SP}}^4 / \Lambda_{\text{SP}}^2} = \frac{|k| M_{\text{Pl}}^2}{a^2 \Lambda_{\text{SP}}^2}$$

#### 2.3.2 The Geometric Suppression Factor
In SPU, the initial spatial curvature is set at the pre-spatial transition where the fermionic condensate crystallizes into a 4D spacetime. At this transition, the relevant length scale is $\ell_{\text{SP}}$, not $\ell_{\text{Pl}}$. The curvature radius of the emerging spacetime is:

$$R_{\text{curv}} \sim \ell_{\text{SP}}^{-1} \cdot M_{\text{Pl}}$$

because the gravitational coupling itself emerges at the scale $\Lambda_{\text{SP}}$. The initial curvature parameter is therefore:

$$|\Omega - 1|_{\text{initial}} \sim \left(\frac{\ell_{\text{SP}}}{R_{\text{curv}}}\right)^2 = \left(\frac{M_{\text{Pl}}}{\Lambda_{\text{SP}}}\right)^{-2} \cdot \mathcal{F}(\delta^*)$$
...
**Factor 1 — The collective enhancement $\sqrt{N_f^{\text{eff}}}$:**
The emergent Planck mass in SPU satisfies:

$$M_{\text{Pl}}^{\text{eff}} = \sqrt{N_f^{\text{eff}}} \cdot f_{\text{IR}} \cdot M_{\text{GUT}}$$

with $f_{\text{IR}} \approx 4.79$ (derived from the Plancherel measure of E₇/SU(8), see `Analisi Analitica del Fattore IRCoset e7su8.md`). Therefore:

$$\frac{M_{\text{Pl}}}{\Lambda_{\text{SP}}} = \frac{\sqrt{N_f^{\text{eff}}} \cdot f_{\text{IR}} \cdot M_{\text{GUT}}}{\sqrt{N_f^{\text{eff}}} \cdot M_{\text{GUT}}} = f_{\text{IR}} \approx 4.79$$
...
$$|\Omega - 1|_{\text{SPU, initial}} \sim \frac{\mathcal{F}(\delta^*)}{f_{\text{IR}}^2} = \frac{0.137}{(4.79)^2} = \frac{0.137}{22.9} \approx 6 \times 10^{-3}$$


### 2.4 Evolution of $|\Omega - 1|$ from GUT Scale to Today
#### 2.4.1 Standard Evolution After the SPU Transition
After the SPU gravitational phase transition at $T \sim M_{\text{GUT}}$, the universe evolves according to standard Friedmann dynamics. The curvature parameter evolves as:
...
| --- | --- | --- | --- |
| Required fine-tuning at $t_{\text{Pl}}$ | $10^{-60}$ | None (inflation resolves it) | None (geometry resolves it) |
...
The range reflects the uncertainty in $N_e$ (10 to 30 e-folds) and in the precise value of $\mathcal{F}(\delta^*)$.
...
1. **Geometric suppression:** The finite vacuum capacity of E₇/SU(8) constrains the initial curvature to $|\Omega-1|_{\text{initial}} \sim \frac{\mathcal{F}(\delta^{\ast})}{f_{\text{IR}}^2} \approx 6\times 10^{-3}$, dramatically smaller than the Planck-scale naive estimate of $\mathcal{O}(1)$.
...
- `Analisi Analitica del Fattore IRCoset e7su8.md` — Derivation of $f_{\text{IR}} \approx 4.79$
- `spu_emergent_cosmological_constant.md` — Spectral vacuum energy
- `consistency_bound_gravity_scale.md` — $\Lambda_{\text{SP}}$ bounds
