...
# -*- coding: utf-8 -*-
"""
IR_Dominance_Calculator.py (v2.0 — Updated May 2026)
...
AGGIORNAMENTO: R0 impostato a 0.895 (valore geometricamente preferito)
secondo i vincoli algebrici derivati da E₇ → E₆ × U(1) e dallo spettro dei Casimir.
...
# Valore geometricamente preferito di R0 (da vincoli algebrici E₇/SU(8))
R0_GEOMETRIC = 0.895
...
def crossover_scale(R0: float, M0: float = M_GUT) -> Optional[float]:
    ...
    mu_star : float or None
        Scala di crossover μ* (GeV), o None se non esiste (R₀ fuori range)
    """
    # Formula corretta derivata da R(μ*) = 1
    numerator = R0 * C2_56 - C2_27      # ✅ R₀·C₂(56) - C₂(27)
    denominator = 1.0 - R0               # ✅ 1 - R₀
...
def plot_running_ratio(R0_values: list, mu_range: Tuple[float, float] = None, 
    ...
    if mu_range is None:
        mu_range = (1e12, 1e19)  # Da 10 TeV a 100 EeV
...
def generate_report(R0: float, M0: float = M_GUT) -> str:
    ...
    report = []
    report.append("=" * 70)
    report.append("SPU IR DOMINANCE ANALYSIS REPORT (v2.0)")
    report.append("=" * 70)
    report.append(f"\nInput parameters:")
...
def main():
    """Esecuzione di esempio con valore geometrico preferito R₀ = 0.895"""
    
    print("\n" + "🔷"*35)
    print("SPU IR DOMINANCE CALCULATOR (v2.0)")
    print(f"Primary hypothesis: R₀ = {R0_GEOMETRIC} (geometrically preferred)")
    print("🔷"*35 + "\n")
    
    # Valori di test: R₀ geometrico + estremi del range ammissibile
    R0_test_values = [0.40, 0.65, R0_GEOMETRIC, 0.95]
    ...
    if valid_pairs:
        ...
        # Valore "geometrico"
        mu_star_geo = crossover_scale(R0_GEOMETRIC)
        if mu_star_geo:
            print(f"\n  Ipotesi geometrica R₀ ≈ {R0_GEOMETRIC} → μ* ≈ {mu_star_geo:.3e} GeV")
            print(f"  → log₁₀(μ*/GeV) ≈ {np.log10(mu_star_geo):.1f}")
            print(f"  → μ*/M_GUT ≈ {mu_star_geo/M_GUT:.2f}")
    ...
    print(f"   R = running_ratio(mu=1e15, R0={R0_GEOMETRIC})")
    print(f"   mu_star = crossover_scale(R0={R0_GEOMETRIC})")
    print(f"   cons = check_consistency(R0={R0_GEOMETRIC})")
